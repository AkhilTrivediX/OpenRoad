...
OAuth refresh-token rotation, provider write-back, direct provider notification delivery, conflict UI, hosted webhook registration automation, SSO/MFA, bulk member operations, and hosted account management remain hardening work that should stay behind server-only secret management, background job controls, and/or explicit delivery infrastructure.
