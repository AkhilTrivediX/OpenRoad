      ...
      release: {
        ...
        rollback: {
          dataMigration: expect.stringContaining(
            "OpenRoad state schema 6; integration metadata schema 4; session metadata schema 2; team metadata schema 5"
          )
        },
        version: "0.1.0-rc.1"
      },
...
async function writeBuildArtifacts(root) {
  ...
  await writeFile(join(root, "server", "integrations.ts"), "export const openRoadIntegrationSchemaVersion = 4;");
  await writeFile(join(root, "server", "session-store.ts"), "export const openRoadSessionSchemaVersion = 2;");
  await writeFile(join(root, "server", "team.ts"), "export const openRoadTeamSchemaVersion = 5;");
