import type { ChangelogItem, RequestItem, RoadmapItem, WorkItem, Workspace } from "./openroad.js";
...
function scoreDuplicateCandidate(
  ...
  const titleOverlap = intersect(tokenSet(selectedRequest.title), tokenSet(candidate.title));
  ...
  const sameRequester = normalizeTerm(selectedRequest.requester) === normalizeTerm(candidate.requester);
