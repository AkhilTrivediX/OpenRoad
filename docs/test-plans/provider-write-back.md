...
- Known unresolved risks: Provider status transitions, label/tag mapping, bulk write-back, hosted webhook registration automation, automatic provider issue creation, and distributed write locks remain later production slices. Conflict UI was completed later in `feat/conflict-resolution-ui`.
