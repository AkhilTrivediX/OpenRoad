...
Provider conflict resolution routes require `integration:manage`, which is reserved for local owners/admins and workspace owners. They resolve only active conflicted issue mappings in the requested workspace and provider. The browser submits only a resolution choice. Accept-provider resolution fetches the current provider issue through server-only GitHub App or encrypted Linear/Jira credentials, applies the existing provider-to-OpenRoad request sync transform, and clears the conflict. Keep-OpenRoad clears the conflict without changing the request. Disconnect-mapping disconnects only the selected mapping. Responses, sync events, and audit events must not return provider tokens, refresh tokens, private keys, ciphertext, authorization headers, raw provider payloads, or webhook secrets.

Hosted webhook registration routes require `integration:manage`, which is reserved for local owners/admins and workspace owners. The browser submits only an installation id. It cannot submit webhook URLs, webhook secrets, provider tokens, SSL flags, event lists, private keys, authorization headers, or provider config. GitHub registration derives the public callback URL and webhook secret from server-only configuration and records bounded sanitized metadata. Linear and Jira registration attempts are blocked unless OpenRoad can verify future provider-created deliveries with a server-known secret. Responses, sync events, audit events, and registration metadata must not return provider tokens, refresh tokens, private keys, webhook secrets, JWTs, ciphertext, authorization headers, raw provider responses, request headers, or request bodies.

Requester notification delivery routes require global `state:write` permission. `POST /api/openroad/notifications/deliver` can use a server-configured JSONL adapter or HTTP provider adapter only; browser requests cannot submit provider URLs, bearer tokens, callback URLs, delivery channels, or provider config. HTTP notification provider delivery derives the endpoint, optional bearer token, and timeout from server-only environment variables, blocks redirects, requires HTTPS except localhost/loopback development URLs, and posts only public-safe queued notification summaries plus workspace context. Delivery responses, persisted notification metadata, audit events, and provider failure text must not return provider bearer tokens, authorization headers, raw provider responses, notification preferences, private workspace state, private changelog notes, internal comments, integration credentials, webhook secrets, or full workspace exports.

Assistant triage routes require `workspace:read` for the requested workspace. `POST /api/openroad/workspaces/:workspaceId/assistant/triage` is advisory and must not mutate OpenRoad state. The browser can submit only a selected `requestId`, an `allowExternalModel` boolean, and explicit consent flags. Provider URLs, API keys, model names, raw prompts, raw provider responses, and provider config are server-only. External model calls require server-side provider configuration plus request-level context-sharing consent; otherwise the route returns deterministic suggestions. Prompt context must be bounded and redacted before leaving the server, must exclude internal/hidden comment bodies, private changelog notes, notification outbox bodies, provider secrets, encrypted credential material, webhook signatures, raw provider payloads, and private workspace exports. Model output is untrusted and may refine only the selected-request problem summary and next action in the foundation slice. Duplicate suggestions, changelog draft suggestions, and all source-of-truth mutations remain deterministic and human-approved. Audit and operational events must not include raw prompts or raw responses.

Operational event routes are private diagnostics, not public product history. `GET /api/openroad/ops/events` requires `state:read` for global reads; when `workspaceId` is supplied, workspace-scoped actors may read only their own workspace events through `workspace:read`. Filters support bounded `limit`, `workspaceId`, `category`, `severity`, and `provider`. `GET /api/openroad/ops/status` requires private state read and returns compact event counts and latest warning/error timestamps, not event bodies. Operational events are stored in team metadata schema `6` and must remain bounded and sanitized. They may include safe actor/request/workspace/provider identifiers and primitive counts/status fields, but must not include provider tokens, authorization headers, webhook secrets, raw provider payloads, raw request/response bodies, stack traces, environment values, encrypted credential internals, session/admin tokens, password material, private workspace exports, private changelog notes, or internal comments. Audit events remain the user/action history; operational events are for production support.

Invitation management routes require `integration:manage`, which is reserved for local owners/admins and workspace owners. Creating an invitation returns the raw accept token exactly once and stores only a hash in `OPENROAD_TEAM_FILE`; invitation records were introduced in team metadata schema `2`, delivery metadata in schema `3`, account credentials in schema `4`, account recovery records in schema `5`, and operational events in schema `6`. When `OPENROAD_INVITATION_DELIVERY_MODE=file` is configured, creation also writes a server-side JSONL delivery handoff record containing the raw accept token and accept URL for an external mail/helpdesk worker. When `OPENROAD_INVITATION_DELIVERY_MODE=http` is configured, creation requires `OPENROAD_PUBLIC_APP_URL` and posts a bounded invitation payload to the configured HTTPS provider URL with the accept URL and optional bearer authorization header. Localhost and loopback `http://` provider URLs are allowed only for development and tests; provider URLs with embedded username/password credentials are rejected. HTTP provider bearer tokens are server-only environment secrets and must never be persisted, returned, logged in errors, or exposed to browser code. HTTP provider mode must not follow redirects, must bound provider response text, and must redact provider message ids and failure text before persistence or API output. List, revoke, accept, session, audit, and ops API responses must not return invitation token hashes or raw accept tokens after creation. Team backups are sensitive restorable snapshots and may contain token hashes, but never raw accept tokens. Built-in SMTP, provider-specific invitation templates, OAuth login, and email verification remain out of scope for this slice.
...
Account recovery request and confirmation routes are public only because the recovery token is the bearer secret. `POST /api/openroad/account/recovery/request` must return the same generic response for eligible users, unknown users, users without account credentials, ambiguous workspace memberships, and disabled recovery delivery. When `OPENROAD_ACCOUNT_RECOVERY_DELIVERY_MODE=file` and a public base URL are configured, eligible requests append one sensitive JSONL record containing the raw recovery token and reset URL for an external worker, while `OPENROAD_TEAM_FILE` stores only the token hash, user id, normalized email, optional workspace id, expiry, consumption, and bounded delivery metadata. `POST /api/openroad/account/recovery/confirm` accepts a valid pending recovery token, sets a new account password, consumes the token, revokes active member sessions for the recovered user, creates a fresh httpOnly member session, and returns only sanitized actor, user, membership, and recovery status metadata. Recovery APIs must not return raw recovery tokens, token hashes, password hashes, salts, submitted passwords, session cookie values, admin tokens, provider tokens, private workspace state, or cross-workspace membership data.

Member management routes require `integration:manage`, which is reserved for local owners/admins and workspace owners. `GET /api/openroad/workspaces/:workspaceId/members` returns only sanitized membership summaries: membership id, user id, workspace id, name, email, role, creation timestamp, local-owner flag, and boolean account-password readiness. `PATCH /api/openroad/workspaces/:workspaceId/members/:membershipId` changes a persisted workspace role, records an audit event, and revokes active workspace-member sessions for that user/workspace so stale cookies cannot keep old permissions. `POST /api/openroad/workspaces/:workspaceId/members/:membershipId/deactivate` removes only that workspace membership, preserves the user and credential record for other memberships or future reactivation, records an audit event, and revokes affected member sessions. Member responses must not return password hashes, salts, session records, session token hashes, admin-token hashes, invitation token hashes, raw invitation tokens, recovery token hashes, raw recovery tokens, provider tokens, encrypted provider payloads, or private workspace state. The local owner bootstrap membership cannot be deactivated or role-changed, and the last owner membership in a workspace cannot be demoted or deactivated.
...
$env:OPENROAD_ACCOUNT_RECOVERY_DELIVERY_FILE=".openroad/openroad-account-recovery-deliveries.jsonl"
